...
std::vector<Rect> generate_rooms(Leaf &bsp, std::vector<std::vector<int>> &grid, std::mt19937 &random_generator) {
    ...
    while (!queue.empty()) {
        ...
        if (current.left && current.right) {
            ...
            // Create a room in the current leaf and save the rect. If a room cannot be created, the width to height
            // ratio is outside of range so try again
...
void place_tile(std::vector<std::vector<int>> &grid, std::mt19937 &random_generator, TileType target_tile,
    ...
    std::uniform_int_distribution<> possible_tiles_distribution(0, (int) possible_tiles.size()-1);
