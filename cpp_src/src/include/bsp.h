...
struct Leaf {
  ...
  bool split(std::vector<std::vector<TileType>> &grid,
    ...
    if (split_vertical_val) {
      ...
      left = new Leaf{Rect{Point{container.top_left.x, container.top_left.y},
                           Point{
                               pos - 1,
                               container.bottom_right.y,
                           }}};
      right = new Leaf{
          Rect{Point{
                   pos + 1,
                   container.top_left.y,
               },
      ...
      left = new Leaf{Rect{Point{container.top_left.x, container.top_left.y},
                           Point{
                               container.bottom_right.x,
                               pos - 1,
                           }}};
      right = new Leaf{
          Rect{Point{
                   container.top_left.x,
                   pos + 1,
               },
               Point{container.bottom_right.x, container.bottom_right.y}}};
    }
  ...
  bool create_room(std::vector<std::vector<TileType>> &grid,
    ...
    std::uniform_int_distribution<> width_distribution(MIN_ROOM_SIZE,
                                                       container.width);
    int width = width_distribution(random_generator);
    std::uniform_int_distribution<> height_distribution(MIN_ROOM_SIZE,
                                                        container.height);
    int height = height_distribution(random_generator);

    // Use the width and height to find a suitable x and y position which can
    // create the room
    std::uniform_int_distribution<> x_pos_distribution(
        container.top_left.x, container.bottom_right.x - width);
    int x_pos = x_pos_distribution(random_generator);
    std::uniform_int_distribution<> y_pos_distribution(
        container.top_left.y, container.bottom_right.y - height);
    int y_pos = y_pos_distribution(random_generator);

    // Create the room rect and test if its width to height ratio will make an
    // oddly-shaped room
    Rect rect =
        Rect{Point{x_pos, y_pos}, Point{x_pos + width - 1, y_pos + height - 1}};
