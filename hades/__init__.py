"""A procedurally generated dungeon crawler made in Python.

This game uses `Arcade <https://api.arcade.academy/en/latest/>`_ to visualise the game
and make it playable, `Numpy <https://numpy.org>`_ for the procedural generation,
`Pymunk <https://www.pymunk.org/en/latest/>`_ for the physics simulation and `Shapely
<https://shapely.readthedocs.io/en/stable/>`_ to speed up various parts of the Arcade
library.

:copyright: (c) 2021-present Aspect1103
:license: MIT, see LICENSE for more details.
"""

__author__ = "Aspect1103"
__license__ = "MIT"
__version__ = "0.0.1"
