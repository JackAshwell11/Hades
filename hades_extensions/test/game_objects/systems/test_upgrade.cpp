...
class UpgradeSystemFixture : public testing::Test {
 protected:
  ...
  void SetUp() override {
    registry.add_system(std::make_shared<UpgradeSystem>(registry));
  }
  ...
  void create_game_object(int value, int max_level) {
    ...
    components.push_back(std::make_unique<Stat>(value, max_level));
    components.push_back(std::make_unique<Upgrades>(upgrades));
