...
class EffectSystemFixture : public testing::Test {
 protected:
  ...
  void SetUp() override {
    registry.add_system(std::make_shared<EffectSystem>(registry));
  }
  ...
  void create_instant_game_object() {
    ...
    components.push_back(std::make_unique<Stat>(100, -1));
  ...
  void create_status_game_object() {
    std::vector<std::unique_ptr<ComponentBase>> components;
    components.push_back(std::make_unique<Stat>(200, -1));
    components.push_back(std::make_unique<StatusEffects>());
    registry.create_game_object(false, std::move(components));
  }
