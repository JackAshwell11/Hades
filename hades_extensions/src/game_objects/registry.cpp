...
int Registry::create_game_object(bool kinematic, std::initializer_list<ComponentBase> components) {
  ...
  if (kinematic) {
    kinematic_objects_[next_game_object_id] = std::make_unique<KinematicObject>(Vec2d{0, 0}, Vec2d{0, 0});
  }

  // Add the game object to the components
  for (const auto& component : components) {
    components_[typeid(component)].insert(next_game_object_id);
    game_objects_[next_game_object_id][typeid(component)] = std::make_unique<ComponentBase>(component);
  }

  // Increment the game object ID and return the current game object ID
  next_game_object_id++;
  return next_game_object_id - 1;
}
...
void Registry::add_system(std::unique_ptr<SystemBase> system) {}
