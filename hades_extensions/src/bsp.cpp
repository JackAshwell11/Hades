...
bool Leaf::split(Grid &grid, std::mt19937 &random_generator, bool debug_game) {
  ...
  // randomly
  bool split_vertical;
  if (container->width > container->height && (container->width >= CONTAINER_RATIO * container->height)) {
    split_vertical = true;
  } else if (container->height > container->width && (container->height >= CONTAINER_RATIO * container->width)) {
  ...
  int max_size = (split_vertical) ? container->width - MIN_CONTAINER_SIZE : container->height - MIN_CONTAINER_SIZE;
  ...
  if (split_vertical) {
    ...
    left = std::make_unique<Leaf>(Rect({container->top_left.x, container->top_left.y},
                                       {pos - 1, container->bottom_right.y}));
    right = std::make_unique<Leaf>(Rect({pos + 1, container->top_left.y},
    ...
    right = std::make_unique<Leaf>(Rect({container->top_left.x, pos + 1},
                                        {container->bottom_right.x, container->bottom_right.y}));
  }
...
bool Leaf::create_room(Grid &grid, std::mt19937 &random_generator) {
  ...
  int width = width_distribution(random_generator), height = height_distribution(random_generator);
  ...
  int x_pos = x_pos_distribution(random_generator), y_pos = y_pos_distribution(random_generator);
