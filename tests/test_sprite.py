...
def test_hades_sprite_init(registry: Registry, floor_texture: Texture) -> None:
...
def test_hades_sprite_negative_position(
    registry: Registry,
...
def test_hades_sprite_empty_textures(registry: Registry) -> None:
    ...
    with pytest.raises(expected_exception=IndexError, match="list index out of range"):
        HadesSprite(registry, (0, GameObjectType.Player), Vec2d(10, 20), [])


def test_hades_sprite_non_existent_texture(registry: Registry) -> None:
...
def test_animated_sprite_init(registry: Registry, floor_texture: Texture) -> None:
...
def test_animated_sprite_multiple_textures(
    registry: Registry,
    floor_texture: Texture,
) -> None:
