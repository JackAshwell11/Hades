...
auto pathfind(const Grid &grid, const cpVect &start, const cpVect &end,
              const std::function<bool(const cpVect &)> &is_not_traversable,
              const std::function<int(const cpVect &, int)> &heuristic_function)
    -> std::unordered_map<cpVect, Connection> {
  ...
  std::unordered_map<cpVect, Connection> neighbours{{start, {.cost = 0, .source = start, .destination = start}}};
  queue.emplace(0, start, start);

  // Loop until we have explored every neighbour or until we've reached the end
  while (!queue.empty()) {
    const cpVect current{queue.top().destination};
    queue.pop();
    ...
    for (const cpVect &neighbour : grid.get_neighbours(current)) {
...
auto calculate_astar_path(const Grid &grid, const cpVect &start, const cpVect &end) -> std::vector<cpVect> {
  ...
  const auto obstacle_check{
      [&grid](const cpVect &neighbour) { return grid.get_value(neighbour) == TileType::Obstacle; }};
  const auto chebyshev_heuristic{
    // TODO: CAN WE USE CPVDIST
      [&end](const cpVect &neighbour, const int cost) { return cost + get_distance_to(neighbour, end); }};
  ...
  for (cpVect current{end}; current != start; current = result.at(current).source) {
...
auto get_furthest_position(const Grid &grid, const cpVect &start) -> cpVect {
  ...
  cpVect furthest_position{.x = -1, .y = -1};
  int max_distance{-1};

  // Explore the grid using the Dijkstra algorithm to find the furthest
  // position from the start
  const auto floor_check{[&grid](const cpVect &neighbour) { return grid.get_value(neighbour) != TileType::Floor; }};
  const auto dijkstra_heuristic{[&max_distance, &furthest_position](const cpVect &neighbour, const int cost) {
