...
/// @param targets - The targets to attack.
void melee_attack(const Registry *registry, const Vec2d &current_position, double current_rotation,
                  const std::vector<int> &targets) {
  // Calculate a vector that is perpendicular to the current rotation of the game object
  const Vec2d rotation = Vec2d(std::sin(current_rotation), std::cos(current_rotation));
...
std::tuple<Vec2d, double, double> ranged_attack(const Vec2d &current_position, double current_rotation) {
  return {current_position, BULLET_VELOCITY * std::cos(current_rotation), BULLET_VELOCITY * std::sin(current_rotation)};
}

// ----- STRUCTURES ------------------------------
std::optional<std::tuple<Vec2d, double, double>> AttackSystem::do_attack(int game_object_id,
...
void DamageSystem::deal_damage(GameObjectID game_object_id, int damage) const {
