...
using ComponentFactory = std::function<void(Registry*, GameObjectID)>;

/// Alias for a factory function that creates components for a game object with a given level.
using LeveledComponentFactory = std::function<void(Registry*, GameObjectID, int)>;
...
const auto bullet_factory{[](Registry* registry, const GameObjectID game_object_id) {
...
const auto enemy_factory{[](Registry* registry, const GameObjectID game_object_id, const int) {
  ...
  registry->add_component<KinematicComponent>(game_object_id, get_hitboxes().at(GameObjectType::Enemy));
...
const auto wall_factory{[](Registry* registry, const GameObjectID game_object_id) {
  registry->add_component<KinematicComponent>(game_object_id, true);
}};

/// Add the components for the floor game object.
const auto floor_factory{[](Registry*, const GameObjectID) {}};

/// Add the components for the goal game object.
const auto goal_factory{[](Registry* registry, const GameObjectID game_object_id) {
  registry->add_component<KinematicComponent>(game_object_id);
}};

/// Add the components for the player game object.
const auto player_factory{[](Registry* registry, const GameObjectID game_object_id) {
  ...
  registry->add_component<KinematicComponent>(game_object_id, get_hitboxes().at(GameObjectType::Player));
...
const auto health_potion_factory{[](Registry* registry, const GameObjectID game_object_id) {
...
const auto chest_factory{[](Registry* registry, const GameObjectID game_object_id) {
  registry->add_component<KinematicComponent>(game_object_id);
}};

/// Add the components for the shop game object.
const auto shop_factory{[](Registry* registry, const GameObjectID game_object_id) {
  registry->add_component<KinematicComponent>(game_object_id);
}};
}  // namespace
...
auto create_game_object(Registry* registry, const GameObjectType game_object_type, const cpVect& position,
  ...
  if (const auto player_ids{registry->get_game_object_ids(GameObjectType::Player)};
    ...
    if (factories.contains(game_object_type)) {
      factories.at(game_object_type)(registry, game_object_id);
    }
    if (leveled_factories.contains(game_object_type)) {
      leveled_factories.at(game_object_type)(registry, game_object_id, level.value());
    }
  } else {
    // Just move the existing player
    game_object_id = player_ids.at(0);
  }
  if (registry->has_component<KinematicComponent>(game_object_id)) {
    const auto game_object_position{game_object_type == GameObjectType::Bullet ? position
                                                                               : grid_pos_to_pixel(position)};
    cpBodySetPosition(*registry->get_component<KinematicComponent>(game_object_id)->body, game_object_position);
  }
