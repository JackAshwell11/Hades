...
class Registry {
 public:
  ...
  void add_component(const GameObjectID game_object_id, Args&&... args) {
    ...
    if (component_id >= components_.size()) {
      components_.resize(component_id + 1);
    }
    const auto component{std::make_shared<Component>(std::forward<Args>(args)...)};
    components_[component_id][game_object_id] = component;
    if constexpr (std::is_base_of_v<KinematicComponent, Component>) {
  ...
  [[nodiscard]] auto has_component(GameObjectID game_object_id) const -> bool {
    ...
    return components_[component_id].contains(game_object_id);
  }
  ...
  [[nodiscard]] auto get_game_object_components(const GameObjectID game_object_id) const {
    ...
    auto components{
        components_ |
        std::views::transform([game_object_id](auto const& component_map) -> std::shared_ptr<ComponentBase> {
  ...
  void add_system() {
    ...
    systems_[system_id] = std::make_shared<System>(this);
  }
 ...
 private:
  ...
  std::vector<std::unordered_map<GameObjectID, std::shared_ptr<ComponentBase>>> components_;
