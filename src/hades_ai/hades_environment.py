...
class HadesEnvironment(Env):
    ...
    def step(
        ...
        # Update the player's rotation to face the nearest enemy
        player = self.game.registry.get_component(self.game.player, KinematicComponent)
        player_position = np.array(player.get_position())
        enemy_positions = np.array(
            [
                self.game.registry.get_component(
                    enemy_id,
        ...
        # Send the current action's press event
        attack_successful = False
        for action_key in self._action_to_key[action]:
            if action_key == MOUSE_BUTTON_LEFT:
                attack_successful = self.game.on_mouse_press(
                    0,
                    0,
                    action_key,
                    0,
                ...
                self.game.on_key_press(action_key, 0)
