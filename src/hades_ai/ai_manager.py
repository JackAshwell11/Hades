...
class EpisodeResults:  # type: ignore[misc]
    ...
    Attributes:
        ...
        wall_distances: The distances to walls for each step.
    ...
    steps: int = 0
    survived: bool = True
    wall_distances: list[NDArray[np.float32]] = field(default_factory=list)
...
def plot_metric(
    ...
    if save_dir:
        plt.savefig(save_dir / f"{labels[1].lower()}.png")
...
def process_episode(*, train: bool = True) -> EpisodeResults:
    ...
    for _ in range(MAX_STEP_COUNT):
        ...
        if observations:
            results.wall_distances.append(observations["distance_to_walls"])
...
def run_dqn(*, evaluate: bool = False) -> None:
    ...
    for episode in range(EVALUATE_EPISODES if evaluate else 1):
        ...
        min_distances.append(np.min(results.wall_distances))
        average_distances.append(float(np.mean(results.wall_distances)))
    ...
    plot_metric(min_distances, ("Minimum Distances to Walls", "Distance"))
    plot_metric(average_distances, ("Average Distances to Walls", "Distance"))
    plot_metric(healths, ("Health Over Time", "Health"))
    plt.show()
